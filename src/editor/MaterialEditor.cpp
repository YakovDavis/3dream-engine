...
void D3E::MaterialEditor::Draw()
{
	...
	if (ImGui::IsItemHovered() && editor_->lmbDownLastFrame && !ImGui::IsMouseDown(ImGuiMouseButton_Left))
	{
		std::string selectedUuid = editor_->editorContentBrowser_->GetTempUuid();
		if (!selectedUuid.empty() && TextureFactory::IsTextureUuidValid(selectedUuid.c_str()))
		{
